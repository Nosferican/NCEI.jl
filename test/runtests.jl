...
if occursin(r"[A-Za-z]{32}", get(ENV, "cdo_token", ""))
    cdo_token = ENV["cdo_token"]
    ...
    println("A cdo_token was not provided which makes remote testing not possible. However, all tests have passed locally.")
    # Doing dummy testing
    @test true
end
